...
instance (Eq k, Hashable k, FromText k, FromJSON v) => FromJSON (Map k v) where
    parseJSON = withObject "HashMap" (fmap (Map . Map.fromList) . traverse f . KeyMap.toList)
      where
        f (k, v) = (,)
            <$> either fail return (fromText $ Key.toText k)
            <*> parseJSON v

instance (Eq k, Hashable k, ToText k, ToJSON v) => ToJSON (Map k v) where
    toJSON = Object . KeyMap.fromList . map (bimap (Key.fromText . toText) toJSON) . toList
